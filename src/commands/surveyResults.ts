  ...
  async execute(interaction: any) {
    ...
    if (!survey) {
      return interaction.editReply(
        `I couldn't find the survey "${surveyName}".`
      );
    }
    ...
    if (specificUserId) {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: 'Progress',
            value: `${answered} / ${total} questions answered`,
            inline: true,
          },
          {
            name: 'Completion',
            value: `${generateProgressBar(pct, 20)}\n${pct}% Complete`,
            inline: false,
          }
        )
    ...
    if (!results) {
      return interaction.editReply(
        "I couldn't calculate the results for that survey."
      );
    }
    ...
    results.questionResults.forEach((qr: any, idx: number) => {
      if (qr.questionType === 'text') {
        ...
        const text =
          samples.length > 0
            ? samples
                .map(
                  (r: any) =>
                    `• "${r.substring(0, 100)}${r.length > 100 ? '...' : ''}"`
                )
                .join('\n')
            : 'No responses';
        ...
        const bars = qr.optionCounts
          .map(
            (oc: any) =>
              `${oc.option}: ${generateProgressBar(oc.percentage, 15)} (${
                oc.count
              })`
          )
          .join('\n');
        ...
        if (userResponses.length > 0) {
          ...
          userResponses
            ...
            .forEach((r) => {
              const answer =
                r.textResponse ||
                survey.questions.find((q) => q.id === r.questionId)?.options?.[
                  r.selectedOption || 0
                ] ||
                'N/A';
              csv += `"${r.question.questionText.replace(/"/g, '""')}","${
                r.question.questionType
              }","${answer.replace(
                /"/g,
                '""'
              )}","${r.timestamp.toISOString()}","${r.updatedAt.toISOString()}"\n`;
            });
        ...
        results.questionResults.forEach((qr: any) => {
          if (qr.questionType === 'text') {
            qr.textResponses.forEach((response: any) => {
              csv += `"${qr.questionText.replace(
                /"/g,
                '""'
              )}","text","${response.replace(/"/g, '""')}",1,N/A\n`;
            });
          } else {
            qr.optionCounts.forEach((oc: any) => {
              csv += `"${qr.questionText.replace(
                /"/g,
                '""'
              )}","multiple_choice","${oc.option.replace(/"/g, '""')}",${
                oc.count
              },${oc.percentage}%\n`;
            });
          }
    ...
    collector.on('collect', async (i: any) => {
      if (i.customId === `refresh_results_${survey.id}`) {
        ...
        refreshEmbed.setDescription(
          `Total Participants: **${
            updatedResults.totalParticipants
          ...
          if (qr.questionType === 'text') {
            const sample =
              qr.textResponses
                ...
                .join('\n') || 'No responses';
            refreshEmbed.addFields({
              name: `📝 Q${idx + 1}: ${qr.questionText}`,
              value: `**${qr.totalResponses} responses**\n${sample}`,
            });
          } else {
            const bars =
              qr.optionCounts
