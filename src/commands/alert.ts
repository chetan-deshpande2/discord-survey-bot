    ...
    try {
      ...
      if (!data) {
        return interaction.editReply(
          `Sorry, I couldn't find market data for **${symbol}**.`
        );
      }
      ...
      let message = `Got it! I'll ping you when **${symbol}** goes **${condition}** **${formatCurrency(
        price
      )}**.`;
      message += `\n\n**Current**: ${formatCurrency(data.price)}`;

      if (data.high24h && data.low24h) {
        message += `\n**24h Range**: ${formatCurrency(
          data.low24h
      ...
      if (interaction.deferred || interaction.replied) {
        await interaction.editReply(
          'Something went wrong while setting your alert. Please try again later.'
        );
      }
    }
  },
