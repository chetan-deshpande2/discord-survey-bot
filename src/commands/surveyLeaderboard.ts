...
export const command = {
    ...
    async execute(interaction: any) {
        ...
        if (responses.length === 0) {
            return interaction.editReply('📊 No survey responses yet. Be the first to participate!');
        }
        ...
        responses.forEach((user: any, index: number) => {
            ...
            const surveysText = `${user.completed} survey${user.completed !== 1 ? 's' : ''}`;
            const questionsText = `${user.answered} question${user.answered !== 1 ? 's' : ''}`;
