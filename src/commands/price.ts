...
export const command = {
    ...
    async execute(interaction: any) {
        ...
        if (!data) {
            return interaction.editReply(`I couldn't find any price data for **${symbol}**. Are you sure the ticker is correct?`);
        }

        const history = await getHistory(symbol);
        const chartUrl = history.length > 0 ? getSparklineUrl(history, data.change24h >= 0 ? 'rgb(0, 255, 0)' : 'rgb(255, 0, 0)') : null;
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '24h Change', value: formatPercentage(data.change24h), inline: true }
            )
            .setTimestamp()
